...
const RenderTargetVisualizer = require('./RenderTargetVisualizer-CyUMzpE7.cjs');
...
const formats = [THREE__namespace.RedFormat, THREE__namespace.RGFormat, THREE__namespace.RGBFormat, THREE__namespace.RGBAFormat];
class FBO {
    // FBO properties
    ...
    constructor({ name, width, height, renderer, camera, properties = [], channels = 4, // Default to RGBA
        ...
        this.read = new THREE__namespace.WebGLRenderTarget(this.width, this.height, {
            minFilter: THREE__namespace.NearestFilter,
            magFilter: THREE__namespace.NearestFilter,
            format: formats[this.channels - 1] || THREE__namespace.RGBAFormat,
    ...
    `;
        ...
        for (const uniformName in this.material.uniforms) {
            ...
            let type = RenderTargetVisualizer.getGLSLType(uniformValue);
            if (type) {
        ...
        const quadGeometry = new THREE__namespace.PlaneGeometry(2, 2);
        const quadMesh = new THREE__namespace.Mesh(quadGeometry, this.material);
    ...
    inject(data, offset = 0) {
        ...
        while (remainingPixels > 0) {
            ...
            const dataTexture = new THREE__namespace.DataTexture(chunkData, pixelsInRow, // Width of the chunk
            1, // Height of 1 pixel (single row)
            THREE__namespace.RGBAFormat, THREE__namespace.FloatType);
            dataTexture.needsUpdate = true;
            this.renderer.copyTextureToTexture(dataTexture, this.read.texture, null, new THREE__namespace.Vector2(x, y));
            // Update counters
...
class PropertyManager {
    ...
    // Basic scene setup
    scene = new THREE__namespace.Scene();
    camera = new THREE__namespace.OrthographicCamera(-1, 1, 1, -1, 0.1, 1000);
    renderer;
    height = 512;
    width = 512;
     ...
     */
    constructor({ renderer = new THREE__namespace.WebGLRenderer(), width = 512, height = 512 }) {
        if (!renderer || !(renderer instanceof THREE__namespace.WebGLRenderer)) {
        ...
        this.camera.lookAt(new THREE__namespace.Vector3(0, 0, 0));
    ...
    constrain(name, constraint) {
        if (!(constraint instanceof Constraint.Constraint)) {
...
class ParticleSystem extends EventTarget {
    ...
    constructor({ canvas, renderer, camera, scene, backgroundColor = [0, 0, 0, 1], fboHeight = 512, fboWidth = 512, plugins = [], }) {
        ...
        // Renderer
        this.renderer = new THREE__namespace.WebGLRenderer({ canvas: this.canvas });
        this.renderer.setClearColor(new THREE__namespace.Color(...backgroundColor.slice(0, 3)), backgroundColor[3]);
        this.renderer.setSize(this.canvas.width, this.canvas.height);
        this.renderer.setPixelRatio(window.devicePixelRatio);
        this.scene = new THREE__namespace.Scene();
        this.camera = new THREE__namespace.PerspectiveCamera(45, this.canvas.width / this.canvas.height, 0.1, 1000);
        ...
        this.particleMaterial = new THREE__namespace.ShaderMaterial({
            vertexShader: particleVertex,
            fragmentShader: particleFragment,
            uniforms: {
                uResolution: { value: new THREE__namespace.Vector2(this.canvas.width, this.canvas.height) },
            },
            transparent: true,
            depthWrite: false,
        });
        this.particleGeometry = new THREE__namespace.BufferGeometry();
        this.particleGeometry.setAttribute("uv", new THREE__namespace.BufferAttribute(this.uvs, 2));
        this.particleGeometry.setAttribute("position", new THREE__namespace.BufferAttribute(new Float32Array(this.maxParticles * 3), 3)); // Placeholder for positions (dummy but required)
        this.renderer.compile(this.scene, this.camera);
        // Only render particle count particles
        this.particleGeometry.setDrawRange(0, this.particleCount);
        this.particles = new THREE__namespace.Points(this.particleGeometry, this.particleMaterial);
    ...
    resize() {
        ...
        this.manager.setUniformsAll({
            u_Resolution: new THREE__namespace.Vector2(this.canvas.width, this.canvas.height),
        });
...
exports.RenderTargetVisualizer = RenderTargetVisualizer.RenderTargetVisualizer;
