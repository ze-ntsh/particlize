...
const textToMesh = async (props) => {
    ...
    geometry.center();
    const material = new THREE__namespace.MeshBasicMaterial({ color: new THREE__namespace.Color().fromArray(color) });
...
const getGLSLType = (value) => {
    ...
    else if (value instanceof THREE__namespace.Vector2) {
        return "vec2";
    }
    else if (value instanceof THREE__namespace.Vector3) {
        return "vec3";
    }
    else if (value instanceof THREE__namespace.Vector4) {
        return "vec4";
    }
...
class RenderTargetVisualizer {
    ...
    constructor(renderer) {
        ...
        this.camera = new THREE__namespace.OrthographicCamera(-1, 1, 1, -1, 0.1, 1000);
        ...
        this.renderer = new THREE__namespace.WebGLRenderer({ canvas: this.canvas, alpha: true });
        this.renderer.setSize(width, height);
        this.renderer.setPixelRatio(window.devicePixelRatio);
        this.labelCanvas = new OffscreenCanvas(256, 64);
        this.labelTexture = new THREE__namespace.CanvasTexture(this.labelCanvas);
        // Resize listener
    ...
    update(fbos) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const geometry = new THREE__namespace.PlaneGeometry(tileSize - spacing, tileSize - spacing);
            const ndcNormalizeMaterial = new THREE__namespace.ShaderMaterial({
                uniforms: {
                    uTexture: { value: new THREE__namespace.DataTexture(fbo.read.texture.image.data, fbo.width, fbo.height) },
                },
                vertexShader: /* glsl */ `
          varying vec2 vUv;
          void main() {
        ...
        `,
            ...
            const mesh = new THREE__namespace.Mesh(geometry, ndcNormalizeMaterial);
...
exports.RenderTargetVisualizer = RenderTargetVisualizer;
exports.deepMerge = deepMerge;
exports.getGLSLType = getGLSLType;
exports.getGLSLValue = getGLSLValue;
